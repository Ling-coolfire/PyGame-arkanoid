...
class Paddle:
    ...
    def activate_power_up(self, type):
        ...
        if type == 'grow':
            ...
            if self.power_up_timers.get('shrink', 0) <= 0:
                current_center = self.rect.centerx
                self.width = 50
                self.rect.width = self.width
                self.rect.centerx = current_center
...
class PowerUp:
    PROPERTIES = {
    'grow':     {'color': (30, 144, 255), 'char': 'G', 'message': 'PADDLE GROW'},     
    'laser':    {'color': (255, 69, 0),   'char': 'L', 'message': 'LASER CANNONS'},
    'glue':     {'color': (50, 205, 50),  'char': 'U', 'message': 'CATCH PADDLE'},
    'shrink':   {'color': (186, 85, 211), 'char': 'K', 'message': 'PADDLE SHRINKS!'},
    'slow':     {'color': (70, 130, 180), 'char': 'W', 'message': 'SLOW BALL'},
    'big_ball': {'color': (255, 215, 0),  'char': 'O', 'message': 'BIG BALL!'},
}
